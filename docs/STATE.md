...
| 项 | 说明 |
|---|---|
| 6d-mask-erode（在跑，优先级 1）| 掩码边界处理机制验证：FastSAM 掩码溢出背景环（面积比 1.02-1.08、有效对应少 11%）——erode3 判负（26.67 崩盘），erode1 甜点档在跑；若回升则掩码后处理可落地 |
| 6d-weak-objects（优先级 2，机制待定）| 上界拆解闭环：候选池非瓶颈；gt_mask +5.84 = 掩码/crop 内容；ape 唯一匹配侧（-1.67）|
