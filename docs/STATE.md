...
| 6d-match-768（已结案 08-12，判负 -18.33）| 分辨率侧收官：768 匹配两版大负（模板不同步 -12.50 / 全修复 -18.33）——查询裁剪 512 是信息上限，768 插值放大污染 desc（模板 patch 尺度同步修复本身有效：Proj +5）；与 superres/1024 闭合，**分辨率不是候选池生成瓶颈的杠杆**，瓶颈在 MASt3R 对应能力本身；附带修复：模板编码与查询长边同步（patch 尺度一致）+ 三处 pix_t 换算 |
