...
| 6d-multi-ext（在跑，08-12）| **multi 泛化失败（分型实锤）**：cat +10.83（池有货）但 ape -10.00 / holepuncher ~-15（池没货）——align_loss 无条件换种子把 best 也换掉（ape 95/120 帧换坏）；gap-oracle 分型（duck/cat 池有货、ape/phone/holepuncher 池没货）完全验证 |
| 6d-multi-gate（todo 08-12）| multi 择优门控（iter_align_multi_gate: 0.05）：候选须比 best 种子相对改善 ≥5% 才替换——通用拒绝域原则，保收益去损失；等 multi-ext 后跑 5 弱物体 |
