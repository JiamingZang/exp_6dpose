...
> 上次更新：2026-08-20 08:30（6d-refviews stage 3 完成，新 champion 79.66；trio 探针启动）
...
| **级联 MEAN（iter_align 2 轮 + 抛光 + 128v 配方，champion）** | **ADD 79.98 / Proj 88.57 / 5cm5° 82.63**（14968 帧；duck 7000 升级 +0.32 修正，08-20）|
| 旧级联 MEAN（64v 配方） | 78.07 / 87.17 / 81.31（hp 干净口径 78.27 修正）|
...
| 结果 | 粗位姿 `outputs/exp_full/results/*.json`；级联 64v `outputs/exp_full_ia/results/*.json`；**级联 128v `outputs/exp_full_v128/results/*.json`**（14968 帧全量，当前冠军口径）|

## 在跑 / 待办

| 项 | 说明 |
|---|---|
| 6d-refviews（结案 08-20）| **新 champion 79.66（+1.59）**：128v 配方全量兑现，升级组 10 正 + cam 持平，零回退；bank 备份 .viewsbak |
| 6d-trio（结案 08-20）| **迭代数物体相关**：duck 7000 全量 49.58（+4.03）升级；ape 持平、hp -9.16 保留 30k；decode-all 搭车判负（预筛验证通过）|
| 6d-refviews64-ctl（结案 08-20）| 归因：cat +15.83 中**视图数 +12.50（79%）**、现代配方 +3.33（21%）——视图数是主因 |
| 6d-viewstruct（结案 08-21）| 模板结构消融判死：V1 16×1 -4.17 / V2 24×1 -4.50 / V3 24×2 +0.50——**roll 冗余是真实覆盖**（ape/hp 极敏感），80t 结构维持；次级发现 duck 24×2 +5.83（视角密度有益）|
| 6d-fillnorm（结案 08-21，**判负**）| 查询裁剪填充归一化（match_fill_norm=0.41）：MEAN 59.17 vs 基线 64.67（**-5.50**）——duck/ape 崩（小物体像素预算 -43%）、cat 0.00（Proj +3.34 匹配变好）、phone +0.84；**查询侧尺度两方向全判负**（放大 768/superres + 缩小 fillnorm）→ bbox+20% 天然尺度=信息最优；代码保留作消融档 |
| 6d-v128-coarse（在跑 08-21，#50 论文取证）| 128v 配方干净粗位姿列：13 物体全量评测中（粗位姿 `dense80_depthc_guided.yaml`，cam/driller `dense80_w1.yaml`，fresh cache `outputs/exp_full_v128_coarse`）——表2 的级联-粗位姿 Δ 需同配方口径 |
| 6d-gsrefiner（已结案 08-17，**判负，先简单验证未跑全量**）| 忠实复刻 GS-Pose GS-Refiner 损失档（纯 (1-SSIM)+(1-MS-SSIM)、lr 5e-3、绝对阈值早停 η=1e-4、关回退保护）：40 帧 duck 逐帧对照**改善 4 / 恶化 28**（旧损失 6/22，更差）；4× 降采样 2/26；三个误差带全净负；粗位姿处损失-误差相关 r=-0.16（负）——**"自洽地错"是渲染比较损失面的系统性属性，换损失救不了**。机制结论：GS-Pose 的 +36.5 refiner 增益来自其弱 init（55.5）的大旋转误差（轮廓信号可见），我们的 iter_align 级联已吃掉旋转误差；剩余单目平移病态对任何 RGB 损失不可见（tz-depth 深度档 +4.17 是唯一有效信号）。**refiner 方向整体结案**：旧损失+回退保护保留（净贡献≈0 靠保护止损） |
...
| 6d-match-768（已结案 08-12，判负 -18.33）| 分辨率侧收官：768 匹配两版大负（模板不同步 -12.50 / 全修复 -18.33）——查询裁剪 512 是信息上限，768 插值放大污染 desc（模板 patch 尺度同步修复本身有效：Proj +5）；与 superres/1024 闭合，**分辨率不是候选池生成瓶颈的杠杆**；**检索拆解闭环（GT 掩码 DINOv2 top-40 失败帧命中 96.7-100%）——正确模板在池内，瓶颈 = MASt3R 稠密对应质量（模型能力极限）**；附带修复：模板编码与查询长边同步（patch 尺度一致）+ 三处 pix_t 换算 |
| 6d-conf-filter（已结案 08-12，判负 -6.67）| 唯一未用信号收官：MASt3R desc_conf 硬过滤判负（v1 双侧 1.5 灾难 4.17%：模板合成图 conf 系统性低 p95≈0.6；v2 查询侧 1.3 仍负：好/坏重叠带大伤数量）；**Proj 85.83 全场最高但 ADD 跌 = 单目平移病态再次印证**；conf 方向结案 |
| 6d-lightglue（已结案 08-13，**判负全灭 0.00/0.00/0.00**）| SuperPoint+LightGlue 稀疏匹配路线收官：duck 120 帧全灭——域差下稀疏对应 ~29 对 → RANSAC-PnP 数值爆炸（t~5e16），渲染择优无法恢复；**对应质量两路线对比结论：MASt3R 稠密 desc+几何先验 >> 零训练稀疏 Transformer 匹配**；与 conf-filter/match-768 三线闭合：对应质量瓶颈不可由换匹配器/调分辨率/滤置信绕过，是模型能力极限；单目侧 RGB 杠杆穷尽（tz-depth 证明真实深度是充分条件）|
