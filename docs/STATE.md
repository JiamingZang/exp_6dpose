...
> 上次更新：2026-08-14 10:05（joint dc2 修复回退：净 -4.00 证伪；消融统一 pre-fix 重跑中）
...
| 项 | 说明 |
|---|---|
| 6d-refviews（在跑 08-17）| 3DGS 训练参考视图数 64→128 隔离探针：**cat +15.83（64.17→80.00）已通过**——30k 实验毒药是迭代数不是配方；扩 9 物体已重训（7000/128v 现代配方），13 物体 120 帧子集门控评测中（~8h）→ 全量评测；待办：duck/ape/hp 30k→7000 探针（迭代数归因尾项）；bank 备份 .viewsbak |
