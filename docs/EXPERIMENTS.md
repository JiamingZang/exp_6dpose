...
## 6d-vggt-recon sanity（VGGT-1B 成对位姿）——判死（08-07）

- 协议：[模板渲染图, 查询真实裁剪图]（GT 位姿裁剪，ape 120 帧）喂 VGGT-1B，
  读 c2w 相对位姿（世界系=模板系），模板距离校准尺度 → 与 GT 比
- 结果：R_err 中位 **94.1°** / tz_ratio 中位 **-0.43（深度反号）** / 方向 111.7° / ADD 823mm
- 消融（域差归零对照）：
  - 相同模板图对：R_err 0.02°（协议本身不崩，合成图可处理）
  - 不同模板视角对：R_err 145°（跨视角旋转预测失败）
- 结论：**VGGT-1B 不能作在线成对位姿求解器**（渲染-真实域差 + 视角差双重失败）；
  VGGT-Omega（2026-05，专门改进单物体/弱纹理）权重 gated 待 HF 授权，授权后可用
  同脚本复跑（只换模型类）
- 教训：VGGT-1B 的同图对完美 ≠ 跨图可用——单物体无纹理合成图在跨视角时位姿
  回归不可靠，域差只是叠加因素

## 任务 1.1 稳定摆放姿态先验（pybullet 自由落体）——通过（08-07）

- 方法：500 随机初始朝向自由落体到台面（pybullet DIRECT，网格碰撞，摩擦 0.9），
  静止姿态按**朝上轴方向**聚类（yaw 是连续族，全旋转距离会把连续族切碎——
  初版按旋转距离聚类得 25-64 簇，改为轴方向聚类后得 3-6 族）
- 稳定姿态族（模型系朝上轴 + 出现次数/500）：
  - duck: 3 族 [0.398,0.837,-0.375](206) [0.427,-0.814,-0.394](213) [z≈1](80)
  - ape: 6 族（含 z≈1 站立族 25 次，其余侧/背躺 35-159 次）
  - cat: 3 族（含 z≈1 站立族 47 次）
  - holepuncher: 通过（中位 4.5°）
  - can: 3 族（z 轴站立族 32 次 + 两个侧躺族）
- 击杀判据（LineMod 全部测试帧 GT 姿态的"朝上轴"是否命中稳定族共识方向）：
  | 物体 | GT 最近角中位 | >15° 占比 | 结论 |
  |---|---|---|---|
  | duck | 5.2° | 0.0% | 通过 |
  | ape | 4.2° | 0.0% | 通过 |
  | cat | 4.9° | 0.1% | 通过 |
  | holepuncher | 4.5° | 0.0% | 通过 |
  | can | 5.5° | 0.1% | 通过 |
- 结论：**测试序列物体确实处于稳定摆放**（GT 朝上轴与重力反方向偏差 <6°），
  稳定先验前提成立 → 进入任务 1.2 接入（软评分，不做硬过滤）
- 附带产出：稳定轴方向 = 近似对称轴分析的基础（任务 4 复用同套网格代码）
