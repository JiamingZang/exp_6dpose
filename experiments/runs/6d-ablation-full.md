...
- `08-15 21:40`：**11_joint 全档定稿（官方 ablation_joint_templates.json）**：
  ...
  **J 曲线双峰形态（J∈{5,12} 并列最高，J∈{10,20} 低点）**：联合 PnP 增益
  集中 J≤5（+4.0），J≥5 为 ±2-3 噪声带（合并集质量取决于进入 top-J 的具体
  模板组合——自洽错误模板混入是概率事件）；逐物体甜点不同：ape/hp 在 J=12
  （46.67/50.83）、cat 在 J=5（60.00）、phone J=5=J=12（65.00）、duck 平坦
  （~32±1.5）；默认 J=12 与 J=5 并列本子集最优（非次优，早前 J=5 缓存聚合
  50.02 与官方 49.33 的差为 RANSAC 末帧噪声，以官方为准）。论文 §5.3.3 加
  J 行、§3.6.3 补 J 曲线一句话。
- `08-15 19:00`：**11_joint J=10 档完整结案 MEAN 46.00**（duck 33.33 /
  ape 40.00 / cat 47.50 / hp 48.33 / phone 60.83）——**J 曲线非单调**：
  45.33(J=1) → 50.02(J=5 峰) → 46.00(J=10 dip) → 49.33(J=12) → J=20 待出。
  J≥5 区间 ±2 噪声带（候选组合噪声：J=10 合并集混入 6-10 位自洽错误模板
  的概率更高）；J=5 为本子集甜点，默认 12 在带内。J=20 后定稿 §3.6.3。
- `08-15 16:00`：**11_joint J=5 档完整结案 MEAN 50.02**（duck 31.67 / ape
  42.50 / cat 60.00 / hp 47.50 / phone 68.42）——**J=5 已超默认 J=12（49.33）**：
  cat +6.67 / phone +3.42 / duck +0.83 反超，ape -4.17 / hp -3.33 落后；
  J 曲线 J=1 45.33 → J=5 50.02 → J=12 49.33（默认在平台/噪声区，非最优）——
  合并集质量受候选组合噪声影响（与 duck K 回退同构）。J=10/20 待出（约 17:10）。
  论文 §3.6.3 将如实写：联合 PnP 增益集中 J≤5，默认 J=12 冗余无害。
- `08-15 14:00`：**11_joint J=1 档完整结案（无联合 PnP）MEAN 45.33**
  （duck 33.33 / ape 32.50 / cat 53.33 / hp 46.67 / phone 60.83）vs J=12
  49.33（-4.00）——联合 PnP 净贡献 +4.00 但**逐物体异质**：ape +14.17
  （弱纹理对应污染单模板解，合并点集变立体显著受益）/ hp +4.16 /
  phone +4.17 / cat 0 / **duck -2.50**（duck 单模板解已干净，合并 top-12
  引入噪声——与 K 曲线 duck K∈[5,20] 回退同构：duck 候选池噪声敏感）。
  §3.6.3"点集变立体后位姿显著更稳"需按物体限定；J=5/10/20 待出（约 14:45）。
...
- `08-15 10:42`：**duck K 曲线验证结案（全新缓存 + 新代码）**：K=1 15.83 /
  ...
  恢复：弱纹理候选池中间密度"自信的错误模板"竞争，联合 PnP（J=12）择优集
  被污染。合并后 5 物体 MEAN：30.50/40.00/42.33/43.33/49.33（K=40 与主表
  口径精确一致）；图重出（thesis_figs/k_curve*.png）；论文 §5.3.3/§5.4 更新。
  链 1 随后启动 consensus。
...
|---|---|---:|---:|---|
| 01 topk | K=40: 49.17 | K=1: 30.50 / K=5: 40.00 / K=10: 42.33 / K=20: 43.33 / K=40: 49.33 | 总体上升（duck K∈[5,20] 非单调回退**复现确认**）| K=40=49.33 与主表口径精确一致；duck 全新缓存重跑：15.83/30.83/27.50/19.17/30.83——K=20 回退是真实现象（弱纹理候选池抖动，联合 PnP 择优集污染），非旧缓存伪影；图已重出；论文 §5.3.3/§5.4 更新 |
...
| 11 joint_templates | J=12（默认）: 49.33 | J=1: 45.33 / J=5: 49.33 / J=10: 46.00 / J=20: 45.83 | J=5 与默认并列最优 | **J 曲线双峰**（J∈{5,12} 最高）：增益集中 J≤5（+4.0），J≥5 噪声带 ±2-3；逐物体甜点不同（ape/hp J=12、cat J=5、duck 平坦）；合并集质量取决于 top-J 模板组合，自洽错误模板混入是概率事件；官方 ablation_joint_templates.json |
