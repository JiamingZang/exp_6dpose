...
| task1-1-stable-prior | done | 1 | `scripts/analysis/stable_pose_prior.py` | 见 EXPERIMENTS.md「任务 1.1」| 测试序列物体是否处于稳定摆放（GT 朝上轴 vs 稳定族共识方向）？| GT 中位角差 >30° 判死 | **通过**：5 物体中位 4-6°（duck 5.2/ape 4.2/cat 4.9/holepuncher 4.5/can 5.5）→ 进入任务 1.2 接入 |
| task1-2-prior-insert | todo | 1 | 接入点 A：`src/detection/localize.py` 排序加权；接入点 B：`src/solver/selection.py` prior 项 | 见任务清单 | 稳定先验软评分能否提升弱物体？| 4 弱物体 ADD ≥+2mm 且 can 不降（±1mm）| A 先做（B 有效再做）；yaw marginalized 避开精确重力方向 |
