...
| 6d-weak-objects | todo | 1 | 待定 | `experiments/runs/6d-weak-objects.md` | 候选池反转后：全解码 +5.0（duck 27.5→32.5），速度代价 matching 3.7→7.4s/帧——全物体验证 + 找不降速的预筛修复 | 任一弱项 +5，MEAN 不降 | 前置：rng 流污染修复（否则 120 帧对比不可信）；ape 匹配侧单独分流 |
