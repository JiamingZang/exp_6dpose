...
| 6d-tz-depth | running | 1 | `configs/current/dense80_depthc_ia_tzdepth.yaml`（champion + tz_search + tz_depth + xy_only）| `experiments/runs/6d-tz-depth.md` | 失败帧 48.5% 旋转对 t 错（tz 35/xy 1556 帧，GT t 替换 100% 恢复）：BOP 深度图掩码内中值替换 z + 渲染质心对齐 xy（FoundationPose 深度初始化启发）能否补上？| duck 120 帧 ≥ 基线+3 且无 -3 回退 → 扩 5 弱物体 | 文献启发（FoundationPose CVPR24 深度中值初始化）；tzxy 质心对齐单独 60 帧滚动 46.67 ≈ 基线（衰减中）|
