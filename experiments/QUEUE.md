...
| 6d-det-align | running | 1 | `configs/current/dense80_depthc_gtbbox.yaml`（待建）| `experiments/runs/6d-det-align.md` | 检测框口径（GT bbox 定位上界）下级联数字是多少，零样本定位代价多大？| 检测框口径 13 物体数字出炉；差值量化定位代价（GSPose 对比通道）| ① GT bbox 定位 + 框内全 1 前景（无 GT mask、无 DINOv2 检索，MASt3R 全解码排序）② 先 120 帧子集验证 ③ 双口径汇报：检测框口径 vs 无检测器端到端 78.07 |
