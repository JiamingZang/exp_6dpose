...
| 6d-loc-upper | running | 1 | `configs/current/dense80_depthc_gtmask.yaml` + `dense80_depthc_sam.yaml` | `experiments/runs/6d-loc-upper.md` | FastSAM 分割是否是弱物体瓶颈（gt_mask 上界下 ADD 提升多少）？| gt_mask 下 duck ADD ≥ +5 → 定位是瓶颈 | **已答**：duck +5.84 / can +5.0 → 定位是瓶颈；SAM 对照判负（+0.83/-3.33，缺口在检测级）；gt_mask 扩展 ape/cat/holepuncher 量化上界中 |
