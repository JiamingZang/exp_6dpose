...
| 6d-loc-upper | running | 1 | `configs/current/dense80_depthc_gtmask.yaml`（gt_mask 定位上界）| `experiments/runs/6d-loc-upper.md` | FastSAM 分割是否是弱物体瓶颈（gt_mask 上界下 ADD 提升多少）？| gt_mask 下 duck ADD ≥ +5 → 定位是瓶颈，改进分割 | 测试时手段穷尽后转定位侧；gt_mask 需重提取 matches |
