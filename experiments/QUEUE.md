...
| 6d-tz-depth | running | 1 | `configs/current/dense80_depthc_ia_tzdepth.yaml` `configs/current/dense80_depthc_ia_tonly.yaml` | `experiments/runs/6d-tz-depth.md` | 失败帧 48.5% 旋转对 t 错（tz 35/xy 1556 帧）：平移病态两档——深度档（BOP 中值 z，消融）/ RGB-only 档（refiner 阶段 1 t-only 加强，可部署）| duck 120 帧任一档 ≥ 基线+3 且无 -3 回退 → 扩 5 弱物体 | 部署实际修正（RGB-only 无深度）：深度档降级消融，主线 t-only；tzxy 质心对齐 60 帧滚动 ≈ 基线（衰减中）|
| 6d-multi-refine | todo | 1 | `configs/current/dense80_depthc_ia_multirefine.yaml`（multi + iter_align_seed_refine_iters: 120）| `experiments/runs/6d-multi-refine.md` | GS-Pose duck 77.2（refiner +38.4）vs 我们优化链 +8.33：把每个种子的优化从匹配+PNP 升级为渲染对比优化（refiner 多种子启动 + 盆底择优）能否兑现差距？| duck 120 帧 ≥ multi 55.83 +3 且无 -3 回退 → 扩 5 弱物体 | refiner 单假设判负过（错误盆+掩码污染）；种子级短精化探盆 + 精化回退保护；效率待实测 |
