...
| 6d-rng-fix | todo | 0 | 代码修复（`src/pipeline.py:561`）| `experiments/runs/6d-rng-fix.md` | self.rng 全局流被逐帧消耗、cache 命中帧跳过 → 120 帧对比抖动 ±6 分；改为每帧确定性种子（如 frame_idx 派生）后 120 帧子集可复现吗？| duck 同 cache 状态两次复跑数字一致；不同 cache 状态数字一致 | **所有 120 帧实验的前置 blocker**；修后历史子集数字需干净复跑 |
| 6d-det-align | todo | 1 | 待新增（外部检测框接入模式）| `experiments/runs/6d-det-align.md` | 换 GSPose 同口径检测框（YOLOv5）喂管线 + 核对评测帧集后，与 GSPose 92.0 可比吗？| 检测框口径 13 物体数字出炉且帧集与 GSPose 论文一致 | 论文主对比表口径；① 核对 GSPose 评测帧集（我们=BOP test 14968 帧）② 检测框接入：框裁剪+框内掩码，跳过 DINOv2 检索（可复用 use_gt_bbox_for_crop 基础）③ 双口径汇报：检测框口径 vs 无检测器端到端（卖点章）|
| 6d-weak-objects | todo | 1 | 待定 | `experiments/runs/6d-weak-objects.md` | 候选池反转后：全解码 +5.0（duck 27.5→32.5），速度代价 matching 3.7→7.4s/帧——全物体验证 + 找不降速的预筛修复 | 任一弱项 +5，MEAN 不降 | 前置 6d-rng-fix；ape 匹配侧单独分流 |
| 6d-prescreen2 | todo | 1 | 待新增（matching 预筛两级化）| `experiments/runs/6d-prescreen2.md` | 两阶段候选筛选（DINOv2 粗召回 top-80 → MASt3R 特征精排 top-40）能否以远小于全解码的速度代价兑现候选池 +5.0？| duck ADD ≥ 全解码（32.5）且 matching 耗时 <5s/帧 | 粗位姿章创新点候选；证据=cand-pool 反转（+5.0）|
| 6d-iter-align | todo | 2 | 待新增（refiner 迭代模式）| `experiments/runs/6d-iter-align.md` | 迭代稠密渲染对齐（当前位姿重渲染→MASt3R 再匹配→重解 PnP，2-3 轮）能否把精化从净负转正？| duck/ape ADD ≥+3 且无大类崩溃 | 位姿优化章创新点候选；位姿越准渲染越像真图→域差随迭代收缩；先 sanity 单帧可视化再上量 |
