...
用法：
    python scripts/eval/run_linemod.py --config configs/current/default.yaml
    python scripts/eval/run_linemod.py --objects ape cat --max-frames 100  # 冒烟
    ...
    for obj in objects:
        ...
        if not bank_path.exists():
            raise FileNotFoundError(
                f"模板库不存在: {bank_path}\n先运行 scripts/data/onboard_object.py "
...
def main():
    ...
    ap.add_argument("--matches-dir", default=None,
                    help="阶段 2 产物目录（scripts/analysis/extract_matches.py 生成，"
