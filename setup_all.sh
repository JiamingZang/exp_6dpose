...
#   bash setup_all.sh data     # 只下数据（内部调 scripts/download_data.sh）
...
run_smoke() {
    ...
    python scripts/run_linemod.py --objects ape --max-frames 20
    echo "冒烟通过。全量实验入口："
    echo "  python scripts/onboard_object.py                 # 全部 13 物体建模板库"
    echo "  python scripts/run_linemod.py                    # 主实验（--bop-csv 可导出 BOP 提交文件）"
    echo "  python scripts/run_ablation.py --help            # 消融"
}
